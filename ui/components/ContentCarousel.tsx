...
const STEP = CARD_WIDTH + CARD_GAP; // 344px per card
...
// Clone the edges so the loop appears seamless
...
export default function ContentCarousel() {
  ...
  // Start at first real card (skip leading clones)
  useEffect(() => {
    if (scrollRef.current) {
      scrollRef.current.scrollLeft = STEP * VISIBLE_CARDS;
    }
  }, []);

  // Teleport instantly when entering clone zones — scroll-smooth must NOT be
  // set via CSS on this element, or the assignment animates instead of jumping.
  const handleScroll = () => {
    const el = scrollRef.current;
    if (!el) return;
    const { scrollLeft } = el;
    if (scrollLeft < STEP * 0.5) {
      // Entered leading clone zone → jump forward into real zone
      el.scrollLeft = scrollLeft + STEP * articles.length;
    } else if (scrollLeft > STEP * (articles.length + VISIBLE_CARDS - 0.5)) {
      // Entered trailing clone zone → jump back into real zone
      el.scrollLeft = scrollLeft - STEP * articles.length;
    }
  };
  ...
  return (
    <section className="relative w-full flex flex-col items-center justify-center py-20 bg-black overflow-hidden">
      ...
      {/* --- Title --- */}
      <h2 className="text-white text-3xl sm:text-4xl font-bold mb-12 text-center z-10 relative">
        Of course we have more content
      </h2>

      {/* --- Scrollable Cards (no scroll-smooth CSS, no snap) --- */}
      <div
        ref={scrollRef}
        className="relative flex gap-6 overflow-x-auto pb-4 w-full z-10"
      ...
      >
        {extendedArticles.map((article, idx) => (
          <div
            key={idx}
            className="flex-shrink-0 bg-neutral-900/95 border border-red-800/70 rounded-2xl shadow-xl w-80 overflow-hidden relative group transition-transform hover:-translate-y-1 duration-200 hover:border-red-600"
          >
      ...
      {/* --- Fade overlays at true left/right edges --- */}
      <div
        className="pointer-events-none absolute left-0 inset-y-0 w-40 z-20"
        style={{ background: "linear-gradient(to right, #000 30%, transparent 100%)" }}
      />
      <div
        className="pointer-events-none absolute right-0 inset-y-0 w-40 z-20"
        style={{ background: "linear-gradient(to left, #000 30%, transparent 100%)" }}
      />

      {/* --- Arrow buttons (above fades) --- */}
      <button
        className="absolute left-4 top-1/2 -translate-y-1/2 z-30 bg-black/60 hover:bg-black/80 rounded-full p-2 border border-white/20 shadow"
      ...
      <button
        className="absolute right-4 top-1/2 -translate-y-1/2 z-30 bg-black/60 hover:bg-black/80 rounded-full p-2 border border-white/20 shadow"
